...
function renderTransactions() {
    ...
    transactionList.innerHTML = records.map((item) => { const account = accountFor(item.accountId); return `<li class="transaction"><span class="transaction__icon">${item.type === "income" ? "↗" : "↘"}</span><div class="transaction__content"><strong>${item.description}</strong><span>${item.category} · ${formatDate(item.date)} · ${account.name}${item.installments > 1 ? ` · ${item.installmentNumber}/${item.installments}` : ""}${item.recurring ? " · mensal" : ""}</span></div><span class="transaction__value transaction__value--${item.type}">${item.type === "income" ? "+" : "−"} ${formatMoney(item.amount)}</span><button class="edit-button" data-edit="${item.id}" type="button" aria-label="Editar">✎</button><button class="delete-button" data-delete="${item.id}" type="button" aria-label="Excluir">×</button></li>`; }).join("");
